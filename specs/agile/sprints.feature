...
Característica: Sprints y daily snapshots del dominio ágil
  ...
  Esquema del escenario: Rechazar la creación de un sprint con datos inválidos
    ...
    Ejemplos:
      ...
      | ""       | hoy          | hoy + 7    | nombre obligatorio        |
      | Sprint 1 | hoy + 7      | hoy        | fecha_fin >= fecha_inicio |
      | Sprint 1 | hoy - 10     | hoy - 3    | fecha_fin >= hoy          |

  @S-AGL-12
  Escenario: Activar y cerrar un sprint siguiendo el ciclo de vida en orden estricto
    Dado que existe un sprint en estado "planning" del "hoy - 2" al "hoy + 5"
  ...
  Escenario: Registrar el daily snapshot de un sprint activo
    Dado que existe un sprint en estado "activo" del "hoy - 2" al "hoy + 5"
    Cuando envío una petición POST a /api/v1/sprints/1/daily_snapshots con:
      ...
      """
    Entonces la respuesta es 201
    Y el snapshot creado tiene fecha "hoy"
  ...
  Esquema del escenario: Rechazar un daily snapshot inválido
    Dado que existe un sprint en estado "<estado_sprint>" del "hoy - 2" al "hoy + 5"
    ...
    Ejemplos:
      ...
      | planning      | hoy     | 21               | 40              | sprint_no_activo  |
      | activo        | hoy - 3 | 21               | 40              | validation_failed |
      | activo        | hoy + 10 | 21              | 40              | validation_failed |
      | activo        | hoy     | -1               | 40              | validation_failed |
      | activo        | hoy     | 21               | -5              | validation_failed |

  @S-AGL-18
  Escenario: Rechazar un segundo snapshot para el mismo día del mismo sprint
    Dado que existe un sprint en estado "activo" del "hoy - 2" al "hoy + 5"
    Y que ese sprint ya tiene un snapshot con fecha "hoy"
    Cuando envío una petición POST a /api/v1/sprints/1/daily_snapshots con:
      """json
      {
        "fecha": "hoy",
  ...
  Escenario: Listar los sprints con su estado y obtener uno por id
    Dado que existen los siguientes sprints:
      ...
      | Sprint 1 | hoy - 14     | hoy - 3    | cerrado |
      | Sprint 2 | hoy - 2      | hoy + 5    | activo  |
...
# contracts:
# - POST /api/v1/sprints: crear sprint (nombre obligatorio, objetivo, fecha_inicio, fecha_fin). Éxito 201 con Sprint; estado inicial "planning". fecha_fin >= fecha_inicio y fecha_fin >= hoy; NO se exige fecha_inicio >= hoy (un sprint puede haber empezado). Violación → 422 validation_failed.
